...
魔塔刷新区域表 = {"九层魔塔一层","九层魔塔二层","九层魔塔三层","九层魔塔四层","九层魔塔五层","九层魔塔六层","九层魔塔七层","九层魔塔八层","九层魔塔九层",}

魔塔陷阱区域表 = {"九层魔塔一层陷阱","九层魔塔二层陷阱","九层魔塔三层陷阱","九层魔塔四层陷阱","九层魔塔五层陷阱","九层魔塔六层陷阱","九层魔塔七层陷阱","九层魔塔八层陷阱","九层魔塔九层陷阱",}

魔塔传送阵刷新表 = {Point(1036, 79),Point(898, 217),Point(761, 354),Point(1156, 199),Point(1018, 337),Point(881, 474),Point(1281, 324),Point(1143, 462),Point(1006, 599),}
...
function 九层魔塔_执行(地图)
    ...
    if 地图.数字变量[数字_陷阱刷新间隔_KEY] == nil then
        地图.数字变量[数字_陷阱刷新间隔_KEY] = 500  -- 与妖塔保持一致
    end
    if 地图.数字变量[数字_魔塔级别] == 0 then
        if 主程.当前时间 > 地图.节点计时 then
            ...
            elseif 地图.副本节点 > 9 then
                ...
                if 节点 == 0 then
                    if 层数 == 1 then
                        地图:地图公告("九层魔塔的封印已解除！ 望各位勇士量力而行，莫要有来无归")
                    elseif 层数 == 2 or 层数 == 3 or 层数 == 4 or 层数 == 6 or 层数 == 7 or 层数 == 8 then
                        地图:地图公告("新一波的魔物即将来袭，请尽快整顿队伍，迎接挑战！")
                    elseif 层数 == 5 or 层数 == 9 then
                        地图:地图公告("一股浓烈的死亡气息正在慢慢逼近，你们会被吞噬吗？")
                    end
                    地图.副本节点 = 地图.副本节点 + 1
                    地图.节点计时 = 主程.当前时间:AddSeconds(3.0)
                elseif 节点 == 1 then
                ...
                elseif 节点 == 3 then
                    ...
                    elseif 层数 == 2 then
                        ...
                        elseif 随机值 == 1 then
                            ...
                            地图:地图公告("两位暴走的守将会互相复活，需要同时将他们击败！")
                            地图:地图公告("两位暴走的守将会互相复活，需要同时将他们击败！")
                        elseif 随机值 == 2 then
                    ...
                    elseif 层数 == 5 then
                        if 随机值 == 0 then
                            ...
                            地图:地图公告("此妖王是 [秘法之囚]，使用法术攻击有奇效！") 
                            地图:地图公告("此妖王是 [秘法之囚]，使用法术攻击有奇效！") 
                        else
                            范围刷新怪物从地图实例("神武之囚01", 地图, 0, Point(1017, 337), true, true)
                            地图:地图公告("此妖王是 [神武之囚]，使用物理攻击有奇效！") 
                            地图:地图公告("此妖王是 [神武之囚]，使用物理攻击有奇效！") 
                        end
                    ...
                    elseif 层数 == 8 then
                        ...
                        elseif 随机值 == 2 then
                            ...
                            地图:地图公告("能力异常的怪物出现了！请冷静应对！")
                            地图:地图公告("这些魔物免疫特定攻击，勇士请注意您的攻击是否有效！")
                            地图:地图公告("这些魔物免疫特定攻击，勇士请注意您的攻击是否有效！")
                        end
                    elseif 层数 == 9 then
                ...
                elseif 节点 == 4 then
                    -- 添加安全检查，确保层数有效
                    if 层数 >= 1 and 层数 <= 9 and 魔塔陷阱区域表[层数] then
                        local 陷阱刷新 = 获取副本怪物区域(地图, 魔塔陷阱区域表[层数])
                        if 地图.存活怪物总数 ~= 0 then
                            -- 使用地图实例的变量而不是全局变量
                            if 地图.数字变量[数字_陷阱刷新间隔_KEY] and 地图.数字变量[数字_陷阱刷新间隔_KEY] > 0 then
                                ...
                                if 陷阱刷新 ~= nil then
                                    ...
                                    if randomNum(1, 100, true) <= 30 then
                                        local 刷新怪物 = 从表随机取值(魔塔陷阱刷新怪物表[层数])
                                        if 刷新怪物 == "落石" then
                                        ...
                                        范围刷新怪物从地图实例(刷新怪物, 地图, 0, 陷阱刷新:获取刷新范围(), true, true)
                                    end
                                    地图.数字变量[数字_陷阱刷新间隔_KEY] = 500
                                else
                                    -- 如果没有陷阱区域，使用怪物区域
                                    local 怪物区域 = 获取副本怪物区域(地图, 魔塔刷新区域表[层数])
                                    if 怪物区域 ~= nil then
                                        if randomNum(1, 100, true) <= 30 then
                                            local 刷新怪物 = 从表随机取值(魔塔陷阱刷新怪物表[层数])
                                            if 刷新怪物 == "落石" then
                                            ...
                                            范围刷新怪物从地图实例(刷新怪物, 地图, 0, 怪物区域:获取刷新范围(), true, true)
                                        end
                                        地图.数字变量[数字_陷阱刷新间隔_KEY] = 500
                                    end
                                end
                ...
                elseif 节点 == 5 then
                    ...
                    for i = 0, 玩家列表.Count - 1 do
                        local 玩家 = 玩家列表[i]
                        玩家.角色数据.零点数字[数字_魔塔个人标准层数记录] = 层数
                    end
                    地图:地图公告("第[".. 层数 .."]波妖物已清除，请点击传送阵通往下层！")
                    地图.副本节点 = 地图.副本节点 + 1
                    地图.节点计时 = 主程.当前时间:AddSeconds(1.0)
                elseif 节点 == 6 then
                    地图.副本节点 = 地图.副本节点 + 1
                    ...
                    if 层数 == 9 then
                        地图:地图公告("恭喜您通过了所有的挑战！请点击传送阵离开魔塔！")
                    else
                        地图:地图公告("传送阵已开启，请点击前往下一层！")
                    end
                end
            end
        end
